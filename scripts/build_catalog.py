...
NON_CANONICAL_PLACEHOLDER_RE = re.compile(
    r"^(?:n/?a|none documented)(?:$|\s*\()",
    re.IGNORECASE,
)


def iter_strings(value, field=""):
    """Yield (field path, string value) pairs from a parsed TOML entry."""
    if isinstance(value, str):
        yield field, value
    elif isinstance(value, dict):
        for key, nested_value in value.items():
            nested_field = f"{field}.{key}" if field else key
            yield from iter_strings(nested_value, nested_field)
    elif isinstance(value, list):
        for index, nested_value in enumerate(value):
            yield from iter_strings(nested_value, f"{field}[{index}]")
...
def main():
    ...
    for path in toml_files:
        ...
        for field, value in iter_strings(entry):
            if NON_ENGLISH_SCRIPT_RE.search(value):
                errors.append((path, f"{field} contains non-English catalog text"))
            if NON_CANONICAL_PLACEHOLDER_RE.match(value):
                errors.append((
                    path,
                    f"{field} uses non-canonical placeholder '{value}'; "
                    "use 'undocumented' or 'not applicable'",
                ))
